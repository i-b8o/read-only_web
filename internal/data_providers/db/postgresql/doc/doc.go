...
func (rs *docStorage) Get(ctx context.Context, docID uint64) (entity.Doc, error) {
	const sql = `SELECT name,abbreviation,header,title,meta,keywords FROM "doc" WHERE id = $1 LIMIT 1`
	row := rs.client.QueryRow(ctx, sql, docID)

	doc := entity.Doc{}
	err := row.Scan(&doc.Name, &doc.Abbreviation, &doc.Header, &doc.Title, &doc.Meta, &doc.Keywords)
	if err != nil {
